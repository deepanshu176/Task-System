...
const getPresenceUserId = (user: PresenceUser) => user._id || user.id || null;
...
export default function LivePresence() {
  ...
  useEffect(() => {
    ...
    const initialActive = users
      .slice(0, 4)
      .map(getPresenceUserId)
    ...
    const interval = setInterval(() => {
      ...
      if (!user) return;
      const userId = getPresenceUserId(user);
      if (!userId) return;
  ...
  return (
    <div className="flex items-center gap-6 px-6 py-2 bg-white border border-slate-100 rounded-2xl shadow-sm">
      ...
      <div className="flex -space-x-2">
        {users.filter((presenceUser) => {
          const userId = getPresenceUserId(presenceUser);
          return Boolean(userId && activeIds.includes(userId));
        }).map((user) => {
          const userId = getPresenceUserId(user);
            ...
            <AnimatePresence>
              {hoveredUser === userId && (
                <motion.div 
                  ...
                  className="absolute left-1/2 top-full mt-3 -translate-x-1/2 px-3 py-1.5 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl whitespace-nowrap pointer-events-none shadow-xl border border-white/10"
                >
